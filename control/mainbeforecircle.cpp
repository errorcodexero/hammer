...
Drivebase_goal swerve_goals(float joy_x, float joy_y, float joy_theta){
	...
	d.mod[0].angle=0;
	d.mod[0].speed=joy_y;
	if (d.mod[0].speed>165)d.mod[0].speed=165;
	if (d.mod[0].speed<-165)d.mod[0].speed=-165;
	d.mod[1].angle=0;
	d.mod[1].speed=joy_y-joy_theta;
	if (d.mod[1].speed>165)d.mod[0].speed=165;
	if (d.mod[1].speed<-165)d.mod[0].speed=-165;
	d.mod[2].angle=0;
	d.mod[2].speed=joy_y+joy_theta;
	if (d.mod[2].speed>165)d.mod[0].speed=165;
	if (d.mod[2].speed<-165)d.mod[0].speed=-165;
	return d;
}
...
Encoder_return current_rotation_direction(Encoder_return encoder_return){//Uses the order of channel outputs for pins two (a) and four (b) to determine wheel rotating direction and estimates the amount of rotational change
	...
	if((encoder_return.time!=0) && (now_a==pre_a && now_b==pre_b)){
		...
		encoder_return.estimated_rotation+=1;
	}
...
Robot_outputs Main::operator()(Robot_inputs in,ostream& cerr){
	...
	estimated_rotation2=encoder_return.estimated_rotation;
	time_place2=encoder_return.time_place;
	if(in.digital_io[1]==DI_0)encoder_return.states.a=0;
	else if(in.digital_io[1]==DI_1)encoder_return.states.a=1;
	if(in.digital_io[2]==DI_0)encoder_return.states.b=0;
	else if(in.digital_io[2]==DI_1)encoder_return.states.b=1;
	...
	switch(status){
		...
		case AUTO_COLLECT:
			if(autonomous_mode){
				...
				return (since_switch>AUTO_DRIVE_TIME)?AUTO_SPIN_UP2DRIVE_WO_BALL:AUTO_COLLECT;
			}
			return COLLECT;
		case AUTO_SPIN_UP2:
			if(autonomous_mode){
